...
function hideInventoryParentChrome() {
  ...
  Array.from(root.querySelectorAll('header, section, div')).forEach((element) => {
    ...
    if (
      ...
      (
        ...
        text.includes('almacenes') ||
        text.includes('movimientos')
      )
    ) {
  ...
  Array.from(root.querySelectorAll('button, a, [role="button"]')).forEach((button) => {
    const text = normalizeText(button.textContent)

    if (inventoryTabs.some((tab) => text === tab)) {
...
function runInventoryParentChromeCleanup() {
  ;[100, 250, 500, 850, 1200, 1800, 2400, 3000].forEach((delay) => {
  ...
  useEffect(() => {
    ...
    const openInventoryProducts = () => {
      ...
      const openedInventory = clickElementByText(
        ['Inventario y Almacen', 'Inventario y Almacén', 'Inventario'],
        ['.sidebar', 'aside', 'body']
      )
      ...
      if (!openedInventory) {
        console.warn('No se encontro Inventario y Almacen.')
        return
      }
    ...
    const handleSidebarClick = (event) => {
      ...
      if (text.includes('inventario') || text.includes('almacen')) {
        event.preventDefault()
        event.stopPropagation()
        event.stopImmediatePropagation?.()
