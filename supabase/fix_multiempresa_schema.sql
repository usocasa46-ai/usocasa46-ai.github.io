-- INVE-FAT SYSTEM - migracion segura de columnas multiempresa.
-- Ejecutar en Supabase SQL Editor cuando PostgREST reporte PGRST204
-- por columnas faltantes como company_code, nombre_comercial o username.
--
-- Este script NO borra datos, NO hace DROP, NO hace TRUNCATE y NO hace DELETE.
...
set
  company_code = upper(coalesce(nullif(company_code, ''), nullif(data ->> 'company_code', ''), nullif(data ->> 'companyCode', ''), nullif(company_id, ''), id)),
  ...
  data = data || jsonb_build_object(
    ...
    'company_code', upper(coalesce(nullif(company_code, ''), nullif(data ->> 'company_code', ''), nullif(data ->> 'companyCode', ''), nullif(company_id, ''), id)),
    'companyCode', upper(coalesce(nullif(company_code, ''), nullif(data ->> 'company_code', ''), nullif(data ->> 'companyCode', ''), nullif(company_id, ''), id))
  ),
  updated_at = now();
...
set
  ...
  max_usuarios = coalesce(license.max_usuarios, nullif(license.data ->> 'max_usuarios', '')::integer, nullif(license.data ->> 'maxUsuarios', '')::integer, 0),
  max_sucursales = coalesce(license.max_sucursales, nullif(license.data ->> 'max_sucursales', '')::integer, nullif(license.data ->> 'maxSucursales', '')::integer, 0),
  max_almacenes = coalesce(license.max_almacenes, nullif(license.data ->> 'max_almacenes', '')::integer, nullif(license.data ->> 'maxAlmacenes', '')::integer, 0),
